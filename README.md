...
| aside | shopee-seller | Shopee Brasil Seller Centre: account check, guarded saves, listings (guided tours, pre-filled attributes, image replacement, a Quill description), listing copy, shop profile, diagnostics, the support chat with Shopee (reaching a human agent, keeping it open, sending an image), with the REPL helpers shipped one file each in [`scripts/`](skills/aside-shopee-seller/scripts/) and a Bun check of live image order |
