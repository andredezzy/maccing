...
context="maccing doctrine: the engineering skills (${doctrine}) and the reasoning skill are STANDING RULES, not suggestions. Before ANY code work — including small or quick edits; that this is just a small change is the exact trap — invoke the matching skill: adding a branch, case, or flag to existing code → dx; creating or splitting files, defining error classes, writing catch blocks, adding helpers or wrappers, structuring tests → organizing-code; designing types, schemas, config or payload shapes → modeling-domains; writing ANY code names things — every variable, function, file, type, or config key an edit introduces, plus boolean vs union vs enum for states → naming, and NEVER coin an identifier before retrieving the framework's, the domain's, or the codebase's existing word; touching any external library, API, or SDK, or fixing any error → researching-before-coding; creating UI components, pages, or layouts, adding props, wiring data fetching for a screen, or building forms → composing-ui. For hard problems, diagnoses, or verdicts → reasoning."
