...
4. If working on ads: ALSO READ every ad-account README
   (.maccing/growth/meta/*/*/ads/*/README.md).
...
**Child skills:** `growth-meta-ads` and `growth-meta-whatsapp` both build on this. If working on ads only, load `growth-meta-ads`. If working on WhatsApp dispatch, load `growth-meta-whatsapp`. Both share the BM infrastructure, isolation rules, and classifier defensive intelligence documented here.
...
**Niche tiers:** Black (offshore casino, unregulated forex) → Gray (recruitment-driven infoproducts) → White (regulated financial services). Legitimate white-category operators differentiate via: real CNPJ, business verification, no guaranteed-return language, visible regulatory registration.
