---
name: database-ops
description: 'Use BEFORE the first read or write against any non-local database, however small: a single-row lookup, a support investigation touching live rows, a data fix, a schema or migration check, an export, or writing up what was done afterwards. Covers the mandatory first step (which host, credentials from the secret store, read or write), the dry-run-and-approval gate on every INSERT/UPDATE/DELETE, the dated case file recording the exact query and its observed output, host identity verification, and deleting exports when the work is finished. MANDATORY, never optional — "quick lookup", "obvious fix", or "it is read-only anyway" is the exact trap: those are the operations that reach production with no record and an unverified host. If the task touches live data at all, load this FIRST. Requires database-mapping before any operation that needs to know which table holds what.'
---
