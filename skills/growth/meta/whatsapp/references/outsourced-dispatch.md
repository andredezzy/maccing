...
Each campaign writes its own short script — 42 to 110 lines in the five written against this package so far, declaring its cells, its controls and the probes it subtracts, then calling `measure` and writing the result beside the script. Those are non-blank, non-comment lines: `grep -cv -e '^[[:space:]]*//' -e '^[[:space:]]*\*' -e '^[[:space:]]*/\*' -e '^[[:space:]]*$' metrics.ts` returns 42, 50, 70, 84 and 110 across the five. Plain `wc -l` on the same five returns 77, 123, 147, 148 and 149, so a reader who counts that way should expect roughly double; the gap is blank lines and the comments that carry each cut's reasoning. Length tracks how many cells a campaign ran rather than how much machinery the script carries: the two-cell scripts come out at 42 and 50, the four-cell one at 70, the seven-cell ones at 84 and 110. A cell is not one line — it names its lists, its phone column, its audience and its exclusions, and the reasoning behind its cut is worth more room than the declaration itself. **This skill layer ships no executable.** It carries the contract and the reasoning; the arithmetic lives somewhere with a build, a version and a changelog.
...
| Two files must actually join | A role's export whose rows reference nobody in the person export is refused. Every row can be well-formed while the column holds the wrong kind of identifier, and the run would otherwise report a matched audience that did nothing. One shared key is enough to pass; this is not a coverage check. It is reported ahead of the blank-column and status checks on the same file, because an export bound to the wrong role usually trips one of those too and their remedies — repopulate the column, widen `valid_statuses` — cannot make the wrong file describe these people. |
...
| A list must be read as declared | A `.txt` carrying a `column` or a `filter`, a column or filter name the header does not carry, a header naming twice a column this cell reads — its own `column` or `filter.column`, or the first column when it declares neither — a quoted field that never closes, and any extension the reader will not guess at are each their own refusal. Every one of them otherwise ends as a short, empty or wrong list that reads as people who did not convert. A repeat among the columns the cell reads nothing from is measured, since the record loses only what nobody was going to look up. |
