...
- [Chip Warming (Unofficial API Route) → see `disposable-bm-strategy.md`](../../growth-meta/references/disposable-bm-strategy.md#chip-warming-unofficial-api-route)
...
- [Number Longevity & Cold Lists → see `disposable-bm-strategy.md`](../../growth-meta/references/disposable-bm-strategy.md#number-longevity--cold-lists-preserve-over-burn)

> **Scope:** WA-specific dispatch operations only. For the full disposable-BM pipeline, proxy/isolation stack, profile acquisition, BM sources, Tier System, Number Warming Protocol (official WABA), Setup Sequence, Display Name Strategy, Business Profile Compliance, and Phone Number Strategy — see the `growth-meta` skill.

### Chip Warming (Unofficial API Route)

> Moved to canonical location — see [`disposable-bm-strategy.md` → Chip Warming (Unofficial API Route)](../../growth-meta/references/disposable-bm-strategy.md#chip-warming-unofficial-api-route).
...
1. **Reset/disable 2FA PIN first** (BM admin can do this WITHOUT the old BSP). The old BSP (e.g. YCloud) sets a 6-digit PIN you don't know — must disable it to re-register elsewhere.
   ...
   - ⚠️ **GOTCHA 2: the 2FA change itself can be BLOCKED by Meta's device-trust hold.** Message: "Você não pode fazer essa alteração no momento. Notamos que você está usando um dispositivo diferente do que costuma usar... Vamos permitir que você faça essa alteração após usar este dispositivo por um tempo." Meta withholds security changes (2FA, password) while the account is being accessed from a device or session it does not recognise for that account. This cascades: can't enable 2FA → can't disable number PIN → can't migrate BSP.
   - **There is no mitigation here and this file will not supply one.** Device trust is a Meta security control. Business Terms §6 forbids acting to "bypass, ignore, or circumvent instructions in our robots.txt file or any measures we employ to prevent or limit access to any part of our Business Services", and §15 states plainly that "Company will not disguise its location through IP proxying or other methods." Conditioning a browser profile until the hold lifts is defeating the control, and that belongs nowhere in a public reference whatever its success rate.
   - **The legitimate route to the same outcome — a released number — never meets the hold.** The two-step PIN was set by the BSP that currently holds the number, and that BSP can disable it server-side. Ask for exactly that, in the same ticket that asks for account restoration: see **"Cleanest unblock when you DON'T know the PIN"** below, and the Gupshup wizard's own wording further down, which states that the *source* BSP must disable 2FA to complete a live-number migration. That route clears the account-2FA gate and the device-trust hold together, because it never touches either.
   - **If the hold is still in the way, the account is being administered from somewhere it has not been administered from before, and the fix is to stop doing that** — sign in as the BM admin from the device and network that account normally uses. The hold measures precisely that and clears on its own once it is true. Where no one can do that, the problem is upstream of this section rather than inside it: Business Terms §3 requires registration information associated with your own Company and warrants that you have not previously been suspended or removed from the Business Services, and §6(c)–(d) prohibit using another user's account and misrepresenting your affiliation with an entity.
...
**Cleanest unblock when you DON'T know the PIN (old BSP set it):** ask the old BSP support to EITHER restore your account OR disable the two-step-verification PIN on the number from their side. The BSP that set the PIN can disable it server-side, so the change is made by the party that owns it and never meets the account-2FA gate or the device-trust hold. Single best ask in a recovery ticket: "Restore access OR disable 2-step PIN on number X."

**NEVER guess the PIN.** Multiple wrong attempts lock the number for hours/days. If you don't know it and can't disable it (device-trust), don't brute-force — recover via the old BSP, which is the route that does not depend on the hold at all.
...
> Moved to canonical location — see [`disposable-bm-strategy.md` → Number Longevity & Cold Lists (preserve over burn)](../../growth-meta/references/disposable-bm-strategy.md#number-longevity--cold-lists-preserve-over-burn).
