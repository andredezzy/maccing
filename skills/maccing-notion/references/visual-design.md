...
4. **Apply the best.** Get each row's page id via `read_database(…, include_ids=true)`, then `PATCH /v1/pages/{id}` `{ "cover": { "type":"external", "external": { "url":"<verified url>" } } }`, then point the gallery at them (`gallery-view.md`).
