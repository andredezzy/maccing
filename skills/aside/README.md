...
| aside-shopee-seller | aside | Shopee Brasil Seller Centre: account check, guarded saves, listings (guided tours, pre-filled attributes, image replacement, a Quill description), listing copy, shop profile, product diagnostics, the support chat with Shopee. The REPL helpers ship as one file each in [`scripts/`](../aside-shopee-seller/scripts/), with a Bun check of live image order |
