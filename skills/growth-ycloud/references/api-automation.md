...
- Response: `data` is an **array, one entry per inbox**; each has `total` (conversation count) and `conversation[]`. Each conversation has `previewMessage` (the last message), `replied` (bool — ⚠️ unreliable, see below), `unreadCount`, **`contact`** (sub-object: `nickName` = WhatsApp pushName, `phoneNumber`, `countryCode`), `assigneeId`, `contactId`, `lastActivityAt` (epoch ms), `contactLastSeenAt`. [verified live 2026-06]
- **Unanswered / "unreplied" = `previewMessage.messageDirection === 0`** (0 = customer spoke last → needs a reply; 1 = we sent last → handled). ⚠️ **Do NOT use the `replied` bool as the unreplied signal** [verified live 2026-06]: it only flips when a **human agent** replies via the Inbox UI — it stays `false` after an **API send** (`POST /v2/whatsapp/messages`), so API-answered conversations keep showing `replied:false` and **linger in the UI's "Unreplied" filter** even though the message was sent + delivered. Use `messageDirection` of the last message instead. Also note `lastActivityAt` = any activity incl. *your* send, while `contactLastSeenAt` = the customer's last activity → use **`contactLastSeenAt`** to measure the 24h window. Phone = `contact.phoneNumber` or `previewMessage.from`; text = `previewMessage.content.text.body` (or `[<contentType>]` for media/button).
