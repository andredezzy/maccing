...
2. **Google Ads MCP** (`growth-google-ads` MCP server) — secondary option when developer token is approved. Same power as Scripts but callable directly from Claude.
...
1. Claude reads the template from the plugin's `skills/growth-google-ads/scripts/` directory
...
- "Profit sharing" + "join/refer" = recruitment-scheme flag
...
All templates in `.maccing/growth/google-ads/<account>/scripts/` (copy from plugin `skills/growth-google-ads/scripts/`).
