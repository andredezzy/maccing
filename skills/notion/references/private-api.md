...
- **Live-verified 2026-06-14** — moved an inline navigation DB to the top of its area page and repositioned 4 spacer paragraphs, one `listAfter` each.
...
- **Live-verified 2026-06-21** — captured the UI op, then set `show_as` bars on 4 rollup props (`Actual <metric>` rollups, `maxValue` = each metric's target, distinct colors) in a daily-totals collection in one transaction; all persisted and render as native bars. This RETIRES the old "native bars are UI-only / not API-settable" guidance.
...
| `500 "MemcachedCrossCellError"` ("Cross-cell memcached access is not allowed") on `getRecordValues` / `syncRecordValues` | A DB **created via the public API** can land in a different Notion **cell** (shard) than the private `api/v3` session, so private **reads** of its collection / blocks / rows hard-fail (memcached is hit cross-cell; the public v1 gateway is unaffected). Undocumented internal Notion error — no public fix; may clear when the cell migrates. **KEY: it blocks private READS only — `saveTransactions` WRITES STILL LAND** (a malformed op returns a normal `ValidationError`, a correct one `200` — NEVER the cross-cell error), and the **public API (v1) works fully** (reads + writes). So run private-only ops on such a collection **BLIND**: issue the `saveTransactions` (template create / set-default, icons, `formula2`, block moves), then **VERIFY via the public API** — read the affected rows / their downstream rollups — instead of `getRecordValues`. Don't fall back to "the collection is inaccessible / do it in the UI." Live-verified 2026-06-24: created a DB default template **and** set it as the collection default on a cross-celled collection entirely via blind `saveTransactions` + a public-API `PATCH` for the template's pre-set relations, confirmed clean via public `read_database` (the aggregations were untouched). |
