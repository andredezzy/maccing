...
- ⚠️ **`listAfter`/`listBefore` reorder a block's POSITION; they do NOT change its `.parent`.** Dropping a block into a *different* page's `content` list via `listAfter` leaves `.parent` still pointing at the origin page — and a **`child_database` / `child_page` whose `.parent` ≠ the page it is displayed on renders as a LINK (a `↗` reference), not inline.** To actually MOVE a database/page to another page so it renders **inline**, **re-parent it** (`PATCH /v1/databases/{id}` `parent`, or `POST /v1/pages/{id}/move`) — re-parenting **appends at the end**, so `listAfter` *after* re-parenting to position it. NEVER cross-page `listAfter` a DB/page and expect inline. (Live-verified 2026-06-19: a restructure that `listAfter`-moved inline DBs into another page WITHOUT re-parenting rendered every one as a page link; `PATCH … parent` on each → inline.)
- ⚠️ **Inline-vs-link is not obvious from a casual read** — a linked DB's content displays identically to a genuinely inline one at a glance (this is what masked the bug above). To confirm a view is genuinely inline, check the block's **`.parent` == the page it's on AND `is_inline: true`** (`GET /v1/blocks/{page}/children` and `GET /v1/databases/{id}`).
- **✅ Reorder workaround — `child_database` / `child_page` blocks only**: re-parent them out and back. Re-parenting **appends at the end** (DBs via `PATCH /v1/databases/{id}` `parent`; pages via `POST /v1/pages/{id}/move`), so to set an order: move each sibling OUT to a temp page, then move them back into the target page **in the order you want** — and leave the block that should be first untouched (everything re-appends after it). Verified live (a financial-tracker restructure). Verify the resulting order with `read_page(page_id, "outline")`, not a raw block-children GET. Plain content blocks (paragraph, callout, embed) can't be re-parented at all — reorder is genuinely UI-only for those.
...
- **Relocating loose blocks** (callouts, headings, dividers, paragraphs, embeds, `column_list`/`column`) — they have no move endpoint, so recreate them at the destination (`PATCH /v1/blocks/{dest}/children`) and trash the originals (`PATCH /v1/blocks/{id}` `{ "in_trash": true }`, or the equivalent `DELETE /v1/blocks/{id}` — both return `in_trash:true`; loose blocks **can** be trashed, they just can't be moved); since both append and move land at the **end**, sequence recreate+move operations one at a time to rebuild the original vertical order. Live-verified in a fitness-area backup restructure (2026-06).
...
- `child_page` and `child_database` blocks **cannot be appended** via `PATCH /v1/blocks/{page_id}/children` — use `POST /v1/pages` or `POST /v1/databases` (after creation, read a single row with `read_page(page_id, "markdown")` or query the database with `read_database`; never a raw `/query` loop — for a row's page `.id`, `read_database(…, include_ids=true)`)
- **Visual spacing between stacked inline DBs** — two inline databases (or any blocks) rendered back-to-back have no gap and read as cramped. Insert an **empty paragraph** between them for breathing room: `PATCH /v1/blocks/{page_id}/children` with `{ "position": {"type":"after_block","after_block":{"id":"<first-db-block-id>"}}, "children":[{ "object":"block", "type":"paragraph", "paragraph":{"rich_text":[]} }] }`. (A `child_database` block's id == its database id.) Live-verified on a financial tracker (two sibling inline DBs) and a daily hub (callout↔first view, view↔view). (*Which* sections deserve a spacer — a callout above its first section, two stacked linked views, a section group and the next — and *why* breathing room matters is design taste: see `visual-design.md` §4.)
