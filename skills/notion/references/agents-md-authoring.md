...
Part of the `maccing-notion` skill — loaded on demand. The SKILL.md MANDATORY rules still apply (sweep, pagination, match-conventions). The sweep rule covers **reading & obeying** AGENTS.md pages; **this file is the complement — how to WRITE and EDIT them well.**
...
- **No, it's general Notion-API mechanism** (a formula pattern, a rollup limitation, a formatter quirk) → one pointer line to the `maccing-notion` skill — already loaded whenever the sweep runs, so re-explaining it here adds no gate value.
