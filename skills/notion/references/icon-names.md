...
Reference for the `maccing-notion` skill — the `name` values for Notion's **built-in icon** type:
...
| chart / fund | `chart-pie` · `chart-line` · `chart-donut` · `chart-area` · `chart-mixed` (⚠ column icon: use `chart-line`) · `chart-alternate` |
| more / other / misc | `more` · `circle` · `grid` · `dashboard` |

### Column icons — silent no-op risk (different from page/db icons)

The names above govern **page & database icons** (public `PATCH /v1/pages|databases/{id}`): a name NOT in the list → immediate `400 Invalid icon name`. **Column/property icons are different** — they go through the private API (`upsert_property`) as an internal `/icons/<file>_<color>.svg` asset path. A name can be **public-valid yet have no matching private asset, so it silently no-ops**: the call returns `200`, no error, and the icon never appears (live-verified 2026-06-14: `chart-mixed` — a **public-VALID** name, `200` from `PATCH /v1/pages`, that nonetheless no-ops as a *column* icon). ⚠️ **A separate trap: a public-INVALID name can ALSO store-but-render-blank as a column icon** — `dumbbell` returns `400 "Invalid icon name"` from `PATCH /v1/pages` (it is NOT in the catalog) yet `upsert_property` stores `/icons/dumbbell_gray.svg` and it renders **blank** in the UI; use `gym` (`gym`/`fire`/`calculator`/`trophy` all render). So the private column-icon path stores ANY `/icons/<name>_<color>.svg` without validating against the public catalog. ⚠️ **The read-back can FALSE-POSITIVE — TWO no-op modes:** **(a)** a name that never stores → a `describe`/`getRecordValues` read shows NO icon path (detected by read-back); **(b)** a name like `dumbbell` **DOES store its `/icons/…_gray.svg` path** — both the `describe` read-back AND `upsert_property` report it `✓` — **yet it renders BLANK in the UI** (verified 2026-06-14: `/icons/dumbbell_gray.svg` stored + read-back `✓`, but the live column header showed no glyph). So a stored-path read-back proves **STORAGE, not RENDERING**: for case (b) the only sure confirmation is a **visual/UI check** (or sticking to a known-good catalog name). This is what the SKILL.md **post-write verification** is for — and for case (b) it must include a visual look, not just the read-back. If a column icon is blank/wrong, swap to a known-good synonym (`gym` for `dumbbell`, `chart-line` for `chart-mixed`) and re-verify.
...
> ⚠️ **This is the ICON palette — `select`/`multi_select` OPTION colors differ on one token each:** options **accept `default`** (icons reject it) and **reject `lightgray`** (icons accept it). Don't reuse an icon color on a select option; use `default` for the lightest chip. (See `pages-properties.md`.)
