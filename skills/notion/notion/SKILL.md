...
**A repeat sweep in the same session may return a marker instead of full text — that still satisfies the Gate.** Once a playbook has been read this session, a later `read_agents_md` call may render it as `[unchanged since this session's earlier sweep · last_edited <iso>] rank N/M · title · id` plus a trailing note that it still governs, instead of re-sending its markdown (the *first* sweep of a session always returns full text). The marker satisfies the Iron Law — the full read already happened and its instructions still stand — but it is never license to skip a playbook you have **not yet read this session**; only a playbook you personally read earlier in this session can ever be marked unchanged.
