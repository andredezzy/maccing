...
- ⚠️ **Inline-vs-link is INVISIBLE to `render_mockup`** — it fetches a linked DB's content and renders it exactly like an inline DB, so a page whose views are actually LINKS looks identical to inline in the mockup (this is what masked the bug above). To confirm a view is genuinely inline, check the block's **`.parent` == the page it's on AND `is_inline: true`** (`GET /v1/blocks/{page}/children` and `GET /v1/databases/{id}`) — never trust the mockup for inline-vs-link.
