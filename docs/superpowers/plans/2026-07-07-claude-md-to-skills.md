...
- [ ] **Step 2: Install ALL skills for the non-Claude agents** — `npx -y skills@latest add andredezzy/maccing -s '*' -g --copy -y -a opencode codex` (agent list = the non-Claude agents in use; verify available agent ids via the CLI's interactive output or `--help` before running; NEVER include claude-code — the plugin serves it).
