...
3. **Phase 3 — cutover:** slim the private `~/.claude/CLAUDE.md` (remove migrated sections, add the bridge line, update MCP tool prefixes in My Accounts), final README pass, version 1.0.0, push, marketplace update.
   - **Other agents' instruction files (verified 2026-07-07):** OpenCode's `~/.config/opencode/AGENTS.md` and Codex's `~/.codex/AGENTS.md` are symlinks to `~/.claude/CLAUDE.md` — the slim cascades to them automatically; no separate cleanup. Two follow-ups: (a) install the maccing skills cross-agent via skills.sh (`npx skills add andredezzy/maccing -a ...` → `~/.agents/skills/`) so the bridge line's "invoke the matching skill" resolves in OpenCode/Codex too; (b) after slimming, re-read the remaining CLAUDE.md once through OpenCode/Codex eyes — Claude-Code-specific leftovers (`/goal`, plugin dev-symlink) are harmless there but must not be load-bearing for those agents.
