...
- **Word budget:** SKILL.md ~500 (all 18 rules in the quick-reference table) + 5 references. Shipped at 610 wc-words under a documented 640 gate — the 18 mandatory rules outrank the 600 convention (2026-07-07).
...
3. **Phase 3 — cutover:** slim the private `~/.claude/CLAUDE.md` (remove migrated sections, add the bridge line, update MCP tool prefixes in My Accounts), final README pass, version 1.0.0, push, marketplace update.
   - **Other agents' instruction files (verified 2026-07-07):** OpenCode's `~/.config/opencode/AGENTS.md` and Codex's `~/.codex/AGENTS.md` are symlinks to `~/.claude/CLAUDE.md` — the slim cascades to them automatically; no separate cleanup. Two follow-ups: (a) **final skills.sh install of ALL skills** (`npx skills add andredezzy/maccing -s '*' -g` targeting the non-Claude agents in use — OpenCode, Codex) so the bridge line's "invoke the matching skill" resolves everywhere; Claude Code is deliberately EXCLUDED from the skills.sh target list — the plugin serves it, and a skills.sh copy would duplicate every skill name. Remove the interim `~/.claude/skills/reasoning` skills.sh copy (installed 2026-07-07 pre-restructure) at the same time, or it collides with the plugin's `maccing:reasoning`. Verify with `npx skills ls -g` (all skills listed) + a fresh Claude session (skills come from the plugin only, no duplicates); (b) after slimming, re-read the remaining CLAUDE.md once through OpenCode/Codex eyes — Claude-Code-specific leftovers (`/goal`, plugin dev-symlink) are harmless there but must not be load-bearing for those agents.
