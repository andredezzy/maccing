...
test("resolveView: resolves visible column property_ids to names via idToName", () => {
  const view: RawView = {
    name: "All tasks",
    type: "table",
    configuration: {
      ...
      properties: [
        { property_id: "title", visible: true },
        { property_id: "CtfH", visible: true },
      ],
    },
  };
  const map: IdToName = { title: "Name", CtfH: "Status" };
  const resolved = resolveView(view, map);
  expect(resolved.name).toBe("All tasks");
  expect(resolved.type).toBe("table");
  expect(resolved.columns).toEqual(["Name", "Status"]);
});

test("resolveView: excludes a column whose visible === false", () => {
  const view: RawView = {
    name: "Filtered",
    type: "table",
    configuration: {
      ...
      properties: [
        { property_id: "title", visible: true },
        { property_id: "CtfH", visible: false },
        { property_id: "M>L>", visible: true },
      ],
    },
  };
  const map: IdToName = { title: "Name", CtfH: "Net worth (last month)", "M>L>": "Net worth (R$)" };
  const resolved = resolveView(view, map);
  expect(resolved.columns).toEqual(["Name", "Net worth (R$)"]);
  expect(resolved.columns).not.toContain("Net worth (last month)");
});

test("resolveView: resolves group_by property_id to a name", () => {
  const view: RawView = {
    name: "Board",
    type: "board",
    configuration: {
      type: "board",
      properties: [{ property_id: "title", visible: true }],
      group_by: { property_id: "sttZ" },
    },
  };
  const map: IdToName = { title: "Name", sttZ: "Status" };
  const resolved = resolveView(view, map);
  expect(resolved.groupBy).toBe("Status");
});

test("resolveView: resolves date_property_id to a name (calendar view)", () => {
  const view: RawView = {
    name: "Calendar",
    type: "calendar",
    configuration: {
      type: "calendar",
      properties: [{ property_id: "title", visible: true }],
      date_property_id: "dtXY",
    },
  };
  const map: IdToName = { title: "Name", dtXY: "Due date" };
  const resolved = resolveView(view, map);
  expect(resolved.dateProperty).toBe("Due date");
});

test("resolveView: falls back to 'View' when view.name is absent", () => {
  const view: RawView = {
    type: "table",
    configuration: { type: "table", properties: [] },
  };
  const resolved = resolveView(view, {});
  expect(resolved.name).toBe("View");
});

test("resolveView: decodes a URL-encoded property_id in the view config before looking it up", () => {
  // View config carries the encoded form "%3F~%5CG"; the idToName map is keyed by the decoded form "?~\G".
  // resolveView calls decodePropertyId on the property_id, so the lookup succeeds.
  const view: RawView = {
    name: "Encoded",
    type: "table",
    configuration: {
      type: "table",
      properties: [{ property_id: "%3F~%5CG", visible: true }],
    },
  };
  // idToName keyed by the decoded form (as buildIdToName produces)
  const map: IdToName = { "?~\\G": "USD Rate" };
  const resolved = resolveView(view, map);
  expect(resolved.columns).toEqual(["USD Rate"]);
});

test("resolveView: groupOptions is always undefined (caller must assign via groupOptionsFor)", () => {
  const view: RawView = {
    name: "Board",
    type: "board",
    configuration: {
  ...
  const resolved = resolveView(view, { sttZ: "Status" });
  expect(resolved.groupOptions).toBeUndefined();
});

// ── groupOptionsFor tests (B) ──────────────────────────────────────────────

test("groupOptionsFor: returns option names in schema order for a status column", () => {
  ...
  expect(groupOptionsFor("Status", schema)).toEqual(["Not started", "In progress", "Done"]);
});

test("groupOptionsFor: returns option names for a select column", () => {
  const schema: PropertiesMap = {
    Priority: {
      id: "prtY",
      type: "select",
  ...
  expect(groupOptionsFor("Priority", schema)).toEqual(["High", "Medium", "Low"]);
});

test("groupOptionsFor: returns option names for a multi_select column", () => {
  const schema: PropertiesMap = {
    Tags: {
      id: "tgXX",
      type: "multi_select",
      multi_select: { options: [{ name: "Frontend" }, { name: "Backend" }, { name: "Design" }] },
    },
  };
  expect(groupOptionsFor("Tags", schema)).toEqual(["Frontend", "Backend", "Design"]);
});

test("groupOptionsFor: returns undefined when groupBy is undefined", () => {
  const schema: PropertiesMap = {
    Status: { id: "sttZ", type: "status", status: { options: [{ name: "Done" }] } },
  };
  expect(groupOptionsFor(undefined, schema)).toBeUndefined();
});

test("groupOptionsFor: returns undefined when the column is not in the schema", () => {
  const schema: PropertiesMap = {
    Name: { id: "title", type: "title" },
  };
  expect(groupOptionsFor("MissingColumn", schema)).toBeUndefined();
});

test("groupOptionsFor: returns undefined when the column has no options (e.g. a plain text property)", () => {
  const schema: PropertiesMap = {
    Name: { id: "title", type: "title" },
  };
  expect(groupOptionsFor("Name", schema)).toBeUndefined();
});
