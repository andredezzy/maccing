...
- **A linked/inline block ALWAYS shows its SOURCE-database name by default** — a gray **`↗ <DB name>`** heading above the tabs (the `↗` marks it a linked view). So a hub/section linked block needs **exactly ONE label**; pick one and suppress the rest:
  - a **section `heading`** above it (the house pattern for stacked daily sections) → then `hide_linked_collection_name:true` on **every** view is **REQUIRED, not taste** — skip it and you get a **redundant double-label** (the heading **plus** the `↗ <DB name>` line);
...
- **⚠️ `render_mockup` CANNOT confirm a linked block's label** (the sibling of the width caveat below) — it prints the `collection_view` block's own empty title as **`◷ Untitled`**, NOT the source-DB name the live UI shows (`↗ <DB name>`). A mockup reading `Untitled` does **NOT** mean the block is unlabeled or named "Untitled" — the live block still shows its source name (or, if hidden, the view-name pill). **NEVER infer "no label → add a heading" from a mockup's `Untitled`** — check the source DB's name or the live UI. Mis-reading this `Untitled` is the #1 cause of an accidental redundant heading. Live-verified 2026-06-23 — a hub's linked log block rendered `◷ Untitled` while the live UI showed `↗ <DB name>`; a heading added on the false "unlabeled" inference produced a double-label until the name was hidden.
...
- ⚠️ **Relative dates resolve in the VIEWER's timezone — and a render/API check resolves them in UTC, so it can be a day off.** The live Notion UI evaluates `"today"`/`"tomorrow"` (and the date-group buckets) against the **account's timezone**; `render_mockup` and API-side relative-date evaluation use **UTC / server time**. During the offset window (e.g. evening in UTC−3, already next-day UTC) they disagree by a full calendar day. Consequences: (1) a row meant for the *user's* tomorrow must be dated the **user's local tomorrow**, not UTC's; (2) **do NOT trust a UTC render to verify a relative-date-filtered view** — it'll bucket rows a day off and show the wrong "Today/Tomorrow." Set dates from the user's local date and confirm in their UI. Live-verified 2026-06-18 (a "Tomorrow" plan rendered empty because the rows sat on the user's tomorrow while UTC had already rolled over).
