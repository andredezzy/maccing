...
- **The view NAME is the on-page section title — don't stack a redundant heading above it.** `hide_linked_collection_name` hides only the gray *database* name; the *view* name still renders as the block's label (a "This week" table view shows a "This week" pill). So a single-purpose linked view on a page needs **no** `heading_3` above it — a heading repeating the view's text is redundant clutter. Name the view well and let it be the title; for breathing room between stacked views, use empty-paragraph spacers (`blocks.md` "Visual spacing"). Live-verified 2026-06-17 (Gym daily hub).
