...
- **Relocating loose blocks** (callouts, headings, dividers, paragraphs, embeds, `column_list`/`column`) — they have no move endpoint, so recreate them at the destination (`PATCH /v1/blocks/{dest}/children`) and trash the originals (`PATCH /v1/blocks/{id}` `{ "in_trash": true }`); since both append and move land at the **end**, sequence recreate+move operations one at a time to rebuild the original vertical order. Verified in the Gym-area backup (2026-06).
