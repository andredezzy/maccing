...
- **Every list-shaped response carries its own `has_more`/`next_cursor` — all are covered:**
  ...
  - `POST /v1/search` — hits
  - `GET /v1/views?data_source_id=` — views (every `read_database` call already dumps each view's full config; hand-roll this only for a write or a standalone per-view GET)
...
| Read a page or DB row — properties **and** body | **`read_page(page_id, format)`** — `markdown` (properties as YAML frontmatter + body) · `outline` (block-id tree with optional `depth`, default 2, for planning edits) · `text` · `mockup` (render the LIVE page as the canonical ASCII page mockup — the one-call way to SHOW how a page looks; optional `depth` tunes the block scan, default 3). Relations→titles, rollups/formulas→scalars, blocks recovered, ~22× smaller than raw JSON. Optional `include_properties=false` suppresses the YAML property frontmatter (default `true`) |
