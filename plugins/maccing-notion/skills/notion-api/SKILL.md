...
| Re-order a database's **view columns** (the per-VIEW column order — distinct from a property's schema/canonical order) | **`order_columns({ data_source_id, order:[names], views?, hide?, show? })`** — reorders the columns of **every** table view of the data source — **including the linked views embedded in other pages** (e.g. per-page "Holdings" views) — or just the given `views`, to `order`. Title pinned first; unlisted columns keep their relative order; **each view's visibility/width is PRESERVED** (filtered views keep their hidden columns) — `hide`/`show` override visibility. The view-write-dual of `read_database`'s # Views. **NB:** order/visibility/width are **view (column)** attributes (`configuration.properties`), NOT property attributes — a property has no public schema order; its *column* in each view does |
