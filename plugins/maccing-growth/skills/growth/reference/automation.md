...
**Read method preference:** When a platform surface supports it, always prefer an in-page `fetch()` via `evaluate-script` over read-only navigation. An in-page fetch has zero behavioral fingerprint and — because no `navigate` call is issued — also avoids the request-sequence-ordering risk entirely (see §3b, §3d, and §3f). Fall back to humanized read-only navigation only when the surface requires a visible render to hydrate state (e.g., a React SPA that fetches data on mount and does not expose the underlying API endpoint). When falling back to navigation, document the reason in a code comment.
