...
| `ControlError` | A pair naming an undeclared cell, naming one cell on both sides, or whose two arms share identifiers; reading an outcome that is not countable or not one its audience is read on; reading a path the record does not carry; or a second control on one treated cell |
