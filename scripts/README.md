...
`--audit` reads every entry aloud and reports three states. **ok**: the line still carries the sentence the entry was written for; the entry also says here if that sentence appears again elsewhere, and if the same term appears in a *different* sentence nobody has covered. **DRIFTED**: the line no longer carries this term, so the entry covers nothing and the hit is back — the message prints where the term is now, and asks you to read that occurrence before moving the entry onto it, because it may be a different disclosure. **STALE**: the file is gone, the path is a directory, the file is not text, no loaded dictionary defines the term under that category any more, or the file no longer contains the term at all. Drifted and stale both fail the audit.
...
Zero false hits begins at **8**, so 16 is twice the measured floor. It costs almost nothing, and the second row is why: above about sixteen characters what survives is no longer the sea of accidental fragments but the genuine ASCII inside a PNG — chunk names, an embedded colour profile — so 64 down to 16 does not quite double the exposed surface, where 4 would have multiplied it by sixty-five. What it buys is that a **short** disclosure is now findable: a twenty-seven-character sentence behind a few NULs used to land in `Not read` while the run exited 0. Scanning entropy is still not harmless — a three-letter term matches random bytes roughly once every few megabytes, and a gate that reports a JPEG's Huffman table as a disclosure is a gate people switch off — which is what the measurement is for, and why the number is 16 rather than 4. **Below sixteen characters, a string inside binary content remains outside what this tool can find.** Re-measure before lowering it further; the twelve blobs above are the test set.
...
- **Bytes it cannot decode.** Inside a file that is not text, only runs of 16 or more decodable characters are read, and a phrase is only matched across a gap of 3 characters or fewer. **A string shorter than sixteen characters inside binary content is not found**, and neither is a term inside compressed or encrypted content, inside an image's pixels, or inside an archive — an archive is opaque here, so a `.zip` or a `.tar.gz` of a private checkout is one file with an innocuous name. A blob over 8 MB in the history is not read at all. Everything in this paragraph is named in the report when it happens, and the verdict says `PASSED WITH GAPS`.
...
- **An exemption is a judgement, not a proof.** Every suppression is somebody's opinion that one sentence is neutral. `--audit` checks that the sentence is still there and tells you when an entry has drifted onto a different one; it cannot check that the opinion was right in the first place.
